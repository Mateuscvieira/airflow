...
class ObjectStoragePath(CloudPath):
    ...
    def copy(self, dst: str | ObjectStoragePath, recursive: bool = False, **kwargs) -> None:
        ...
        get_hook_lineage_collector().add_input_dataset(context=self.store, uri=str(self.path))
        get_hook_lineage_collector().add_output_dataset(context=dst.store, uri=str(dst.path))
    ...
    def move(self, path: str | ObjectStoragePath, recursive: bool = False, **kwargs) -> None:
        ...
        if self.samestore(path):
            get_hook_lineage_collector().add_input_dataset(context=self.store, uri=str(self))
            get_hook_lineage_collector().add_input_dataset(context=path.store, uri=str(path))
